...
void Group::InitListBox(CListBox& studentsList)
{
    for_each(Students.begin(), Students.end(), [&studentsList](shared_ptr<Student>& student)
        {
            studentsList.AddString(student->GetSurname());
        });
    studentsList.SetCaretIndex(0);
}

void Group::UpdateListBox(CListBox* studentsList)
{
    int ind = studentsList->GetCaretIndex();
    studentsList->ResetContent();
    for_each(Students.begin(), Students.end(), [&studentsList](shared_ptr<Student>& student)
        {
            int i = (*studentsList) ? studentsList->GetCount() : 0;
            studentsList->InsertString(i, student->GetSurname());
        });
    studentsList->SetCaretIndex(ind);
}

void Group::ShowStudentInDialog(MyDialog& dialog, const int& index)
{
    Students[index]->ShowInDialog(dialog);
}
