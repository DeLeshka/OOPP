...
// kondrikovOOPP.cpp: определяет поведение классов для приложения.
//
...
BEGIN_MESSAGE_MAP(CkondrikovOOPPApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CkondrikovOOPPApp::OnAppAbout)
...
// Создание CkondrikovOOPPApp

CkondrikovOOPPApp::CkondrikovOOPPApp() noexcept
{

	// TODO: замените ниже строку идентификатора приложения строкой уникального идентификатора; рекомендуемый
	// формат для строки: ИмяКомпании.ИмяПродукта.СубПродукт.СведенияОВерсии
	SetAppID(_T("kondrikovOOPP.AppID.NoVersion"));

	// TODO: добавьте код создания,
	// Размещает весь важный код инициализации в InitInstance
}

// Единственный объект CkondrikovOOPPApp

CkondrikovOOPPApp theApp;


// Инициализация CkondrikovOOPPApp

BOOL CkondrikovOOPPApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CkondrikovOOPPDoc),
		RUNTIME_CLASS(CMainFrame),       // основное окно рамки SDI
		RUNTIME_CLASS(CkondrikovOOPPView));
	if (!pDocTemplate)
		return FALSE;
...
// Обработчики сообщений CkondrikovOOPPApp
...
void CkondrikovOOPPApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}
